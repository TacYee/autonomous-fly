...
def evaluate(net, test_dataset, output_file, use_adf=False, use_mcdo=False):
    ...
    with torch.no_grad():
        for inputs, targets in test_loader:
            ...
            print(outputs_variance, data_variance, model_variance_nm )
    ...
    plt.fill_between(np.arange(len(all_variances.cpu().numpy())), 
                     all_outputs.cpu().numpy().flatten() - np.sqrt(all_variances.cpu().numpy()).flatten() * 20, 
                     all_outputs.cpu().numpy().flatten() + np.sqrt(all_variances.cpu().numpy()).flatten() * 20, 
